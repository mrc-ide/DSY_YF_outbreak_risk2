...
orderly2::orderly_resource(c("enviro_data_IAregions_6covs_old_labelling_ts_error.csv",
                           "markov_chain_data_combined_paper.Rds"))

orderly2::orderly_artefact("FOI and R0 values", "DSY_1000_datasets_FOI_R0.Rds" )
orderly2::orderly_artefact("Other parameter values", "DSY_1000_datasets_FOI_R0.Rds" )

library(YEPaux)

enviro_data=read.csv(file="enviro_data_IAregions_6covs_old_labelling_ts_error.csv",header=TRUE)
regions=enviro_data$region
n_regions=length(regions)
n_env_vars=ncol(enviro_data)-1
env_vars=colnames(enviro_data)[1+c(1:n_env_vars)]

chain_data=readRDS(file="markov_chain_data_combined_paper.Rds")
...
saveRDS(FOI_R0_values,file="DSY_1000_datasets_FOI_R0.Rds")
saveRDS(list(regions=regions,p_rep_severe=chain_data_selected$p_rep_severe,vaccine_efficacy=chain_data_selected$vaccine_efficacy,
             p_rep_death=chain_data_selected$p_rep_death),file="DSY_1000_datasets_additional.Rds")