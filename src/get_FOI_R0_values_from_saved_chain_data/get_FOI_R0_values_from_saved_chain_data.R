...
pars = orderly2::orderly_parameters(n_param_sets=10,
...
orderly2::orderly_shared_resource("enviro_data.csv" = pars$enviro_filename,
                           "markov_chain_data.Rds" = pars$chain_filename)
...
interval=floor(n_entries/pars$n_param_sets)
lines=c(1:pars$n_param_sets)*interval
...
FOI_values_array=array(data=FOI_R0_values$FOI,dim=c(n_regions,pars$n_param_sets))
R0_values_array=array(data=FOI_R0_values$R0,dim=c(n_regions,pars$n_param_sets))
