pars = orderly2::orderly_parameters(p_severe_inf=0.12, p_death_severe_inf=0.39, deterministic=FALSE, 
                                    n_sets_to_run=NULL, n_reps=1, mode_parallel=FALSE, n_cores=1)
...
if(pars$deterministic){assertthat::assert_that(pars$n_reps==1)}
...
for(set in selection){
  ...
  dataset_single <- Generate_Dataset(FOI_values = array(0,dim=c(n_regions,1)),
                                     ...
                                     p_severe_inf = pars$p_severe_inf, p_death_severe_inf = pars$p_death_severe_inf,
                                     p_rep_severe = 1.0,p_rep_death = 1.0,mode_parallel = pars$mode_parallel,
                                     cluster = cluster, output_frame = TRUE, seed = set)
  if(set==selection[1]){
...
datasets_all$cases=datasets_all$severe_cases/pars$p_severe_inf
